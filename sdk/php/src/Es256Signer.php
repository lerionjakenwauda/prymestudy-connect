...
final class Es256Signer
{
    public static function jwt(array $claims, string $privateKeyPem, string $keyId): string
    {
        ...
        if (! is_array($details) || ($details['type'] ?? null) !== OPENSSL_KEYTYPE_EC) {
            throw new RuntimeException('PrymeStudy Connect ES256 requires an EC private key.');
        }

        $curve = $details['ec']['curve_name'] ?? null;
        if (! in_array($curve, ['prime256v1', 'secp256r1'], true)) {
            throw new RuntimeException('PrymeStudy Connect ES256 requires a P-256 (prime256v1/secp256r1) private key.');
        }

        $derSignature = '';
        if (! openssl_sign($input, $derSignature, $key, OPENSSL_ALGO_SHA256)) {
            throw new RuntimeException('Unable to sign PrymeStudy Connect client assertion.');
        }

        return $input.'.'.self::base64Url(self::derEcdsaToJose($derSignature, 32));
    }
    ...
    private static function joseEcdsaToDer(string $raw, int $partLength): string
    {
        ...
        $s = substr($raw, $partLength, $partLength);
        $r = self::derInteger($r);
        $s = self::derInteger($s);
    ...
    private static function normalizeInteger(string $value, int $length): string
    {
        ...
        if (strlen($value) > $length) {
            throw new RuntimeException('ECDSA integer exceeds expected width.');
        }
